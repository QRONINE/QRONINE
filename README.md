<!-- ================= HEADER ================= -->

<h1 align="center"> NILIN MAROLI</h1>
<h3 align="center">Robotics • Embedded Systems • Mechatronics</h3>

<p align="center">
  <a href="mailto:sunilkumarnilinm@gmail.com">📧 Email</a> •
  <a href="https://linkedin.com/in/nilin-m">💼 LinkedIn</a> •
  <a href="https://github.com/QRONINE">💻 GitHub</a> •
  <a href="YOUR_PORTFOLIO_LINK">🌐 Portfolio</a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=QRONINE&label=Profile%20Views&color=0e75b6&style=flat" />
</p>

---

<!-- ================= HERO ================= -->

## 🧠 Who I Am  

I build **intelligent machines** — systems that sense, decide, and act.  

From robotic arms to UAVs to embedded medical systems, I work at the intersection of:  
⚙️ Hardware • 💻 Software • 🤖 Intelligence  

I don’t just write code or design circuits —  
I **engineer complete systems end-to-end**.

---

<!-- ================= CURRENT ================= -->

## 🚀 Currently  

-  Building **ROS 2 robotic systems with IK + motion planning**  
-  Working on **closed-loop actuator control systems**  
-  Exploring **Reinforcement Learning in robotics**  
-  Deep diving into **Embedded Linux systems**  

---

<!-- ================= PROJECTS ================= -->

## 🔥 Featured Projects  

---

### 🤖 IGOR — Intelligent Robotic Arm  
`ROS 2 • IK • Control Systems • Embedded Integration`

🚀 Highlights:
- 3DOF robotic manipulator with **1.5mm precision**
- Custom **Inverse Kinematics solver (Python)**
- **ROS 2 trajectory planning + MoveIt2**
- Closed-loop control using:
  - AS5600 encoders (I2C via TCA9548A MUX)
  - NEMA 17 steppers

🧠 Next:
- Reinforcement Learning-based chess automation  

---

### 🚁 RUDRA — Autonomous VTOL UAV  
`ArduPilot • YOLOv5 • Jetson Nano • Aerodynamics`

🚀 Highlights:
- Hybrid VTOL UAV with **autonomous transitions**
- Real-time fire detection (**30 FPS YOLOv5**)
- Optimized aerodynamics:
  - **+15% lift-to-drag ratio (XFLR5)**

---

### 🏥 VitalNest — IoT Healthcare System  
`MQTT • Embedded Systems • Cloud Integration`

🚀 Highlights:
- Real-time vitals monitoring (SpO2, Pulse, BP)
- Cloud dashboard (**Supabase**)
- Precision drug delivery system:
  - Peristaltic pumps + TMC2209 drivers
  - **0.1 ml accuracy**

---

### 🎮 BMO — Embedded Gaming Console  
`Embedded Linux • Raspberry Pi • Hardware Design`

🚀 Highlights:
- Custom handheld console
- Device Tree + GPIO mapping
- Custom:
  - Class-AB audio circuit
  - Power system
- Fully 3D printed enclosure

---

<!-- ================= TECH ================= -->

## 🛠️ Tech Stack  

### 💻 Languages  
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-000000?style=for-the-badge&logo=c)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java)

---

### ⚙️ Embedded & Robotics  
![STM32](https://img.shields.io/badge/STM32-03234B?style=for-the-badge)
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=for-the-badge)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=for-the-badge)
![Raspberry Pi](https://img.shields.io/badge/RaspberryPi-C51A4A?style=for-the-badge)
![ROS2](https://img.shields.io/badge/ROS2-22314E?style=for-the-badge)

---

### 🧰 Tools  
![MATLAB](https://img.shields.io/badge/MATLAB-FF6F00?style=for-the-badge)
![SolidWorks](https://img.shields.io/badge/SolidWorks-FF0000?style=for-the-badge)
![KiCAD](https://img.shields.io/badge/KiCAD-314CB6?style=for-the-badge)
![Fusion360](https://img.shields.io/badge/Fusion360-FF6F00?style=for-the-badge)

---

### 🔌 Protocols  
`I2C` `SPI` `UART` `MQTT` `BLE` `WiFi`

---



<!-- ================= ACHIEVEMENTS ================= -->

## 🏆 Achievements  

- 🥉 Engineering Clinics II — **3rd / 566 teams**  
- 🥈 Engineering Clinics I — **2nd / 530 teams**  
- 📜 RoboAI Industrial Certification  
- 📜 Embedded Systems — Maven Silicon  
- 📜 UAV Design — Feynman Aerospace  

---

<!-- ================= LEADERSHIP ================= -->

<!--## 🧭 Leadership  

**ACM Student Chapter — Community Manager**  

- Led a **15+ member team**  
- Increased participation by **20%**  
- Delivered **100% on-time events**  

-->

<!-- ================= QUOTE ================= -->

## ⚡ Philosophy  

```text
If it doesn’t move, I make it move.
If it moves, I make it smarter.
```
<!-- ================= CONTACT ================= -->
## 🤝 Let’s Build

```text
I’m open to:

Robotics Internships

Embedded Systems Roles

Mechatronics Opportunities

📧 sunilkumarnilinm@gmail.com
```
<!-- ================= FOOTER ================= --> 
<p align="center"> ⭐ If you like my work, consider starring my repositories! </p> 
